...
L.D3SvgOverlay = (L.version < "1.0" ? L.Class : L.Layer).extend({
    ...
    onAdd: function (map) {
        ...
        this.projection = {
            ...
            transition: function(path, marker, zoom) {
                path.transition()
                    .duration(function(d) {return d.properties.dist4 * 10000})
            ...
            toLine: function(geom, zoom) {
                zoom = _layer._undef(zoom) ? _layer._zoom : zoom;
                const toline = d3.line()
                    .curve(d3.curveLinear)
                    .x((geom) => _layer.map.project(new L.LatLng(geom[1], geom[0]), zoom)._round().x)
                    .y((geom) => _layer.map.project(new L.LatLng(geom[1], geom[0]), zoom)._round().y);
